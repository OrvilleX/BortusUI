...
import { Shortcut } from 'element-ui/types/date-picker'

export const calendarBaseShortcuts: Shortcut[] = [{
  text: '今天',
  onClick(picker) {
...
export const calendarMoveShortcuts: Shortcut[] = [{
  text: '‹ 往前一天 ',
  onClick(picker) {
    const value = picker.value as Date[]
    if (value.length === 0) {
  ...
  onClick(picker) {
    ...
    let end = new Date()
    const value = picker.value as Date[]
    if ((picker.value as Array<Date>).length > 0) {
      if (end.getTime() - value[1].getTime() > 8.64E7) {
