...
import { IUserData, IUserQueryData, IUserDtoData } from "@/types/user";
...
let userRoles: IRoleData[] = [];
let userJobs: IJobData[] = [];
...
>(CRUD) {
  ...
  private handleNodeClick(data: IDeptDtoData) {
    if (data.pid === 0) {
  ...
  // 改变状态
  private changeEnabled(data: IUserData, val: string) {
    this.$confirm(
      '此操作将 "' +
  ...
  private checkboxT(row: IUserData, rowIndex: number) {
