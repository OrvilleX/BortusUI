...
router.put('/', (req: Request, res: Response) => {
    let data = req.body as UserData
    if (data.id && data.username && data.phone && data.nickName && data.email && data.dept && data.jobs) {
        res.json({
            ok: 'ok'
        })
    } else {
...
router.post('/', (req: Request, res: Response) => {
    let data = req.body as UserData
    if (data.username && data.phone && data.nickName && data.email && data.enabled && data.dept) {
        res.json({
            ok: 'ok'
        })
    } else {
